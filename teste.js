...
async function getTesouroInfo(tipoTitulo, vencimentoTitulo) {
  ...
  const end = new Date('2023-03-04')
  console.log(start, end)
  const pus = [];
  let total = 0
  for (let i = 1; i < rows.length; i++) {
    const row = rows[i].split(";");
    const dataBase = row[headers.indexOf("Data Base")];
    ...
    if (
      ...
      && dataBaseDate >= start && dataBaseDate <= end
    ) {
...
(async function () {
  try {
    ...
    for (const bond of treasuryBonds.TrsrBdTradgList) {
      ...
      const dt = await getTesouroInfo("Tesouro IPCA+", "15/05/2035");
